...
2026-09-21 项目扩展修正：native_square的polish_scale/polish_translate现为None保留原扰动，显式数值才在关闭Mosaic时修改增强链；patience=0禁用平台早停。v14当前未启用轻增强收尾，仍保留scale0.3/translate0.1；早停可能早于关闭Mosaic。正式验证协议与预测统一内容裁框，最佳轮图存best_validation。详见[v14执行记录](../归档/02_v9至v14复盘.md#record-7)。以下官方参数正文不改写。

2026-09-20 最新v14：原生1280五通道，保留v13数据与增强；扩展字段early_backbone_lr=2e-5仅作用第1–10层，首层/颈部/头lr0=1e-4；patience40及10/20/50/75/100/125/150/175/200轮历史最佳AP95门槛0.32/0.34/0.36/0.38/0.40/0.45/0.50/0.55/0.60。分层学习率与门槛属于项目扩展，不是直接传给model.train的框架参数。v13后期过拟合，门槛只限制预算，不保证提分。本机原生RandomHSV跳过非三通道，不能只设置HSV非零就宣称五通道有RGB颜色增强；分类dropout/erasing也不用于detect。详见[v13复盘与v14阶段筛选](../归档/02_v9至v14复盘.md#record-6)。以下“当前”均按历史日期范围理解。

2026-09-20 当前v11：沿用v10全尺寸早期融合配置，仅将`cls_pw`由0.25改为0；训练器新增第10/20轮历史最佳AP95门槛0.31/0.34。首轮不设0.4硬门槛，因为v4首轮仅0.29976。门槛属于项目训练器扩展，并非Ultralytics原生参数。未执行测试、训练或推理，详见[v10复盘与v11筛选方案](../归档/02_v9至v14复盘.md#record-3)。

2026-09-20 同日后续：完整参数布局保留，v10现按用户多数原图设置imgsz=1920、配方image_height=1080，真实输入1920×1088；不是imgsz=1080，也不是1920方形训练。120轮余弦、batch2/nbs16、patience30；close_mosaic=80使第41轮转入完整画面，扩展配方polish_scale=0.1/polish_translate=0.025保留轻增强；HSV0.01/0.15/0.15由自定义组件仅作用RGB。下方1280/200轮为被替代草案。无测试或模型任务。

2026-09-20 当前v10：完整显式参数布局继续保留，模型改为五通道首层早期融合，1280、200轮余弦、AdamW统一lr0=1e-4、Mosaic0.5/scale0.3/translate0.1/HSV0、cls_pw0.25，首轮起patience40；非检测任务参数仍不参与训练。本轮未重跑配置/确定性测试，不能沿用v9单批测试结论。具体依据见[v9复盘与v10训练方案](../归档/02_v9至v14复盘.md#record-2)；下面v9和更早设置均为历史。

2026-09-19 当前 v9：用户要求保留完整参数展示。main.py 已显式列出本机 Ultralytics 8.4.152 的 default.yaml 全部配置键，并额外指定 trainer、save_dir、augmentations；完整列出不等于全部启用。模型、数据、资源、学习率、损失和同步增强沿用 [v9方案](../归档/02_v9至v14复盘.md#record-1)，下方 v4/v5/v8 描述均为历史。dfl=1.5 在 YOLO26 中实际控制 L1 框距离损失，不能当作无效兼容项。蒸馏关闭时 dis=6.0 不参与训练；导出、跟踪、姿态、分割、深度估计参数保持默认值，不作用于当前检测训练。

自定义实现边界：RectangularDataset 固定完整划分、12 类和矩形输入，只实现 Mosaic、尺度、平移、水平翻转和 RGB HSV。其他原生增强参数列出不代表已实现；fraction/classes/agnostic_nms 等入口值不能视为可随意调节的开关。调这些行为须同步修改实现和训练/预测一致性约束。

2026-09-18 适用说明：本文为Ultralytics默认参数与历史YOLO项目覆盖记录，不是当前main.py配置。D-FINE v8使用独立TrainingConfig、官方原生损失和optimizer_step日程，不能把YOLO的box/cls/dfl或Mosaic参数当成等价设置。当前值见 [v8方案](../归档/01_早期训练与清洗.md#record-7)，通用知识见 [知识索引](../项目知识索引.md)。下方v4/v5“当前”均限于历史阶段。

v5_full 当前覆盖：lr0=0.0001（骨干 0.2 倍）、cls_pw=0、mosaic=0.25、scale=0.2、translate=0.05、close_mosaic=4840、patience=200、nms=True、save_period=50；弱类采样、原尺寸同步裁剪与 RGB 独立增强属于训练优化.py 的项目实现，不是框架新增参数。第 161 轮同时关闭 Mosaic、目标裁剪、辅助模态缺失。完整配置见 [v5方案](../归档/01_早期训练与清洗.md#record-2)，下方 v4 说明为历史记录。

项目说明：下表保留抓取时的框架默认值，不是当前训练配置。v4 的实际值见 main.py 与 [训练配置与数据集复核](../归档/04_数据与调参经验.md#record-2)：YOLO26l、五通道、1280、batch=4、workers=4、AdamW、lr0=0.0001、nbs=16、MAX_EPOCHS=5000、patience=100；已使用清洗数据完成训练。
