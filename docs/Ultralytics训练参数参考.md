...
2026-09-21 项目扩展修正：native_square的polish_scale/polish_translate现为None保留原扰动，显式数值才在关闭Mosaic时修改增强链；patience=0禁用平台早停。v14当前未启用轻增强收尾，仍保留scale0.3/translate0.1；早停可能早于关闭Mosaic。正式验证协议与预测统一内容裁框，最佳轮图存best_validation。详见[v14执行记录](v14一致性修复与执行记录.md)。以下官方参数正文不改写。

2026-09-20 最新v14：原生1280五通道，保留v13数据与增强；扩展字段early_backbone_lr=2e-5仅作用第1–10层，首层/颈部/头lr0=1e-4；patience40及10/20/50/75/100/125/150/175/200轮历史最佳AP95门槛0.32/0.34/0.36/0.38/0.40/0.45/0.50/0.55/0.60。分层学习率与门槛属于项目扩展，不是直接传给model.train的框架参数。v13后期过拟合，门槛只限制预算，不保证提分。本机原生RandomHSV跳过非三通道，不能只设置HSV非零就宣称五通道有RGB颜色增强；分类dropout/erasing也不用于detect。详见[v13复盘与v14阶段筛选](v13复盘与v14阶段筛选.md)。以下“当前”均按历史日期范围理解。

2026-09-20 当前v11：沿用v10全尺寸早期融合配置，仅将`cls_pw`由0.25改为0；训练器新增第10/20轮历史最佳AP95门槛0.31/0.34。首轮不设0.4硬门槛，因为v4首轮仅0.29976。门槛属于项目训练器扩展，并非Ultralytics原生参数。未执行测试、训练或推理，详见[v10复盘与v11筛选方案](v10复盘与v11筛选方案.md)。
...
2026-09-20 当前v10：完整显式参数布局继续保留，模型改为五通道首层早期融合，1280、200轮余弦、AdamW统一lr0=1e-4、Mosaic0.5/scale0.3/translate0.1/HSV0、cls_pw0.25，首轮起patience40；非检测任务参数仍不参与训练。本轮未重跑配置/确定性测试，不能沿用v9单批测试结论。具体依据见[v9复盘与v10训练方案](v9复盘与v10训练方案.md)；下面v9和更早设置均为历史。

2026-09-19 当前 v9：用户要求保留完整参数展示。main.py 已显式列出本机 Ultralytics 8.4.152 的 default.yaml 全部配置键，并额外指定 trainer、save_dir、augmentations；完整列出不等于全部启用。模型、数据、资源、学习率、损失和同步增强沿用 [v9方案](v9融合训练实施.md)，下方 v4/v5/v8 描述均为历史。dfl=1.5 在 YOLO26 中实际控制 L1 框距离损失，不能当作无效兼容项。蒸馏关闭时 dis=6.0 不参与训练；导出、跟踪、姿态、分割、深度估计参数保持默认值，不作用于当前检测训练。
