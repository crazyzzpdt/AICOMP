...
本文记录 v6 架构引入时的实施方案，1280/60轮、初始batch=2等为当时配置。当前v8使用1280、batch=4/有效16、主学习率5e-5/骨干5e-6、早停，数据1709/291；见 [v8方案](v8训练方案与数据清洗.md)。D-FINE的原生查询排序不能替换成v4的YOLO NMS。当前用户预测入口选择v7 best_map50.pth、1536；换模型须显式匹配尺寸。
