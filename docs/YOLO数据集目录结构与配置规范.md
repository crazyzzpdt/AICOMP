...
2026-09-18 最新目录约定：datasets根目录仅train、val与data.yaml，当前1709/291，每个划分含images、infrared、depth、labels。旧1744/256及缓存已完整搬入v8审计的before备份；新目录未混用旧缓存。D-FINE使用相同数据目录及独立清洗标签，不使用YOLO标签/NPY缓存训练机制。硬链接图像禁止原地修改。详细留痕见 [v8方案](v8训练方案与数据清洗.md)，下方v4段落为历史。
