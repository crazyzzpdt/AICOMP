...
调整批次与输出位置：
    uv run python predict.py --batch 8 --output predict_v7_fast
使用历史 YOLO 权重：
...
if __name__ == "__main__":
    predict(PredictionConfig(
        # 一、模型、数据与输出
        weights=MODEL_PATH,  # 用户指定的训练权重；D-FINE 使用同一轨迹的 EMA
        source=SOURCE_PATH,  # 只读官方三模态图像，不生成缓存或修改原始文件
        output=OUTPUT_PATH,  # 保存带框图片、六列标签和提交 ZIP，已有目录不覆盖
        backend="auto",  # .pth 使用 D-FINE，.pt 使用历史 YOLO 五通道预测器
        ...
        prefetch_batches=10,  # 按当前 batch 预取 10 批，具体队列容量由预测组件计算
        pin_memory=True,  # D-FINE 当前批次使用锁页内存，uint8 传输后在 GPU 转 FP32
        device="0",  # 本机 RTX 5080；保持 FP32，不默认使用半精度或 INT8

        # 三、检测候选与后处理：保持比赛口径，不用降低精度换速度
        conf=0.001,  # 保留低分候选计算 AP，与验证一致，不是图片展示阈值
        iou=0.7,  # 仅用于历史 YOLO 的 NMS；D-FINE 使用原生查询排序，不使用此值
        max_det=100,  # 赛事每图最多 100 框，按置信度保留，不做多模型集成
        multi_label=False,  # YOLO 默认单标签；D-FINE 原生查询类别排序不受此开关控制
        yolo_profile="v4",  # YOLO 恢复历史逐张原生 NMS；当前 D-FINE 权重忽略此项
