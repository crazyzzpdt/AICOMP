...
2026-09-19 开训提醒：当前main.py的IMAGE_HW已改为(1920,1080)，但imgsz仍取第二项1080，与内容宽1920冲突，需同步索引后再训练。本次仅记录，不代改参数。v9保留完整的数据清洗、开训审计、融合训练及提交流程；57文件清洗已留痕，额外约200张人工复核仍待完成，详见[v9实施说明](docs/v9融合训练实施.md)。未运行测试、训练或推理。
...
当前运行名为`AIC_RGBIRDepth_yolo26l_1920x1080_v9_fusion`，重名由YOLO递增目录。best.pt按mAP50-95选取，另存best_map50.pt、last.pt和周期检查点。只恢复同配方中断任务；已完成/早停的last.pt拒绝恢复。新机器需自行准备同版本依赖和官方数据；不要无故升级本机已适配的框架。

训练完成确认实际权重路径后运行`uv run python predict.py`，输出到独占新目录`predict_v9`。历史模型预测须显式传权重和原尺寸。
