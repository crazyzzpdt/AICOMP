...
## 当前：v18未改善，已完成模态数值审计（2026-09-22）
...
训练1709组IR/Depth已做只读数值审计：PNG/JPG深度采样零值约27.65%/80.92%，5张JPG深度全像素为0；PNG采样没有超过20000的值，量程有官方依据。部分IR目视存在黑边/模糊/轮廓残差，尚无全量错位率。下一候选为[质量感知、局部容错融合设计](docs/superpowers/specs/2026-09-22-quality-aware-fusion-design.md)，待审核后实施。

**main.py仍是已完成v18，不要重复开训；v19尚未实现。** 预测用户默认未改，选择候选时显式指定权重、复赛source与新输出目录。现有清洗1709/291、标签和代码不变，本轮没有测试或模型任务；只提交文档，不上传原图/逐图输出。不保证0.60或赛事60分，旧版候选保留。
