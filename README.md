...
后续更新：训练入口已切换v26_illumination，在v25浮点流程上增加训练专用IR Gamma/受限局部对比度、RGB曝光扰动；网络仍为五通道共享骨干，不是三分支光照门控。验证和旧权重推理不新增增强。仅代码修改，未测试或启动模型；详见[实施方案](docs/浮点三模态实施方案.md)。下方v25为基础配方说明。
